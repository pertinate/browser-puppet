...
const puppetInstance_1 = require("./puppetInstance");
const getCurrentPage = async (pages, timeout = 100000) => {
...
const moveAndClick = async (page, x, y) => {
    console.log("test");
    await page.evaluate(() => {
...
setTimeout(async () => {
    setInterval(() => { }, 1000);
    const browser = await puppetInstance_1.getBrowser();
    const pageNames = await Promise.all((await browser.pages()).map((element) => element.title()));
